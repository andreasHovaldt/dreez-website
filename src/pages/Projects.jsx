...
function Projects() {
    ...
    return (
          <>
            ...
            <div>
              <GreyBoxProjects title="4th Semester: Foundation Models in Agricultural 3D Perception" 
                              description="Applying 2D and 3D Visual Foundation Models to In-Field LiDAR Detection and Segmentation. The project encompassed capturing synchronized LiDAR, camera, and RTK GPS data, developing an efficient annotation pipeline, and training on this custom dataset as well as the established ScanNet v2 dataset, using SOTA 3D detection and segmentation models, while also exploring the effects of the most recent 2D and 3D visual foundation models."
                              abstract="The growing demand for food production and a shortage of skilled machine operators motivate research into agricultural automation, and with it, agricultural 3D perception. While agricultural perception has largely centred on image-based modalities, and 3D perception on indoor and autonomous-driving scenarios, agricultural 3D perception remains underexplored. This thesis investigates whether modern foundation models can be applied to this setting. To enable the study, a custom LiDAR dataset was captured on private fields and manually annotated, with the public ScanNet v2 dataset used in parallel to anchor the findings to existing literature. Within a 3DETR-based detection framework, two foundation models are evaluated: the point-cloud model Utonia as a detection encoder, and the image model DINOv3, whose per-point embeddings are injected into the 3D representation. Multi-task detection and segmentation training is additionally explored. Fine-tuning Utonia to the agricultural domain gives the strongest detection configuration by a wide margin, raising mAP25 from 14.4 to 57.0 over an equivalent from-scratch encoder, whereas a frozen Utonia encoder provides no benefit. Injecting frozen DINOv3 embeddings produces the single largest improvement, lifting the mAP25 of from-scratch models from 6.1 and 14.4 to 50.5 and 53.0, while raw RGB input gives no reliable gain. Multi-task training offers little practical advantage and consistently degrades segmentation. Overall, agricultural 3D perception benefits substantially from foundation models, both domain-adapted point-cloud models and injected image features.">
              </GreyBoxProjects>
            </div>
